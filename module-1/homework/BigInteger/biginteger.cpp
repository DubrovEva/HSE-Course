...
BigInteger operator+(const BigInteger& first, const BigInteger& other){
    ...
    if (first.sign != other.sign) {
        if (first.sign) {
            ...
            another.sign = true;
            result = first - another;
        } else {
            BigInteger another = first;
            another.sign = true;
        ...
        bool carry = 0;
        for (size_t i = 0; i < std::max(first.figures.size(),(other.figures).size()) || carry; ++i) {
            if (i == result.figures.size())
                result.figures.push_back(0);
            int a = (i < first.figures.size() ? first.figures[i] : 0);
            int b = (i < other.figures.size() ? other.figures[i] : 0);
            result.figures[i] =  a + carry + b;
            carry = result.figures[i] >= BigInteger::base;
            if (carry)  
                result.figures[i] -= BigInteger::base;
        }   
    }
...
BigInteger operator-(const BigInteger& first, const BigInteger& other){ 
    ...
    if (first == other)
        result = BigInteger(0);
    else if (first.sign && !other.sign) { // a - -b
        BigInteger another = other;
        another.sign = true;
        result = first + another;
    } else if (!first.sign && other.sign) { //-a - b
        ...
        if (first > other)  {
            ...
            for (size_t i = 0; i < other.figures.size() || carry; ++i) {
                ...
                if (carry)  result.figures[i] += BigInteger::base;
            }
            ...
            bool carry = 0;
            for (size_t i = 0; i < first.figures.size() || carry; ++i) {
                result.figures[i] -= carry + (i < first.figures.size() ? first.figures[i] : 0);
                carry = result.figures[i] < 0;
                if (carry)  result.figures[i] += BigInteger::base;
            }
            while (result.figures.size() > 1 && result.figures.back() == 0)
...
BigInteger operator*(const BigInteger& first, const BigInteger& other){
    BigInteger result;
    result.sign = (first.sign && other.sign) || (!first.sign && !other.sign);
    result.figures.resize(first.figures.size() + other.figures.size());
    for (size_t i = 0; i < first.figures.size(); ++i)
        for (int j = 0, carry = 0; j < other.figures.size() || carry; ++j) {
            long long cur = 
                result.figures[i + j] + first.figures[i] * 1ll * (j < (int)other.figures.size() ? other.figures[j] : 0) + carry;
            result.figures[i + j] = int (cur % BigInteger::base);
...
BigInteger operator/(const BigInteger& first, const BigInteger& other){
    bool carry = 0;
    BigInteger l = 1, r = first, diff = BigInteger(1);
    ...
    l.sign = (first.sign && other.sign) || (!first.sign && !other.sign);
...
bool operator==(const BigInteger& first, const BigInteger& other) {
    return (first.figures == other.figures) && (first.sign == other.sign);
}
bool operator!=(const BigInteger& first, const BigInteger& other) {
    return !(first == other);
}
bool operator>(const BigInteger& first, const BigInteger& other) {
    ...
    if (first.sign && !other.sign) 
        return true;
    else if (!first.sign && other.sign)
        return false;
    else if (first.sign && other.sign) {
        if (first.figures.size() != other.figures.size()) {
            return first.figures.size() > other.figures.size();
        } else {
            for (int i = first.figures.size() - 1; i >= 0; --i) {
                if (first.figures[i] != other.figures[i])
                    return first.figures[i] > other.figures[i];
            }
            return false;
        }
    }
